...
| ID | P | Item | Milestone |
|---|---|---|---|
| DEBT-029 | P1 | **The dashboard depends on two Streamlit APIs whose removal dates have already passed** | Found 2026-07-30 by running the app after the M2 step 2.1 extraction; unrelated to it. Streamlit 1.58 prints both on every startup: `use_container_width` (**33 uses** in `app.py` + `pages/journal.py`) was to be removed after **2025-12-31**, and `st.components.v1.html` (**1 use**) after **2026-06-01**. Both dates are in the past — the dashboard runs only because the compatibility shims have outlived their notice, and any Streamlit upgrade may remove them without further warning. Not urgent while the version is pinned, but it means **the version is now effectively pinned whether or not that was decided**. **Fix:** `use_container_width=True` → `width='stretch'`, `=False` → `width='content'`; `st.components.v1.html` → `st.iframe`. Mechanical, but touches 34 call sites, so it wants doing in one pass — natural fit with `views/` (2.4), where most of the call sites will be moving anyway. Revisit at M5.0 if the migration-away-from-Streamlit question reopens. | M2.4 |
| DEBT-028 | P2 | **`core/` still uses the private names it was extracted with** | Opened 2026-07-29 by ADR-032. Every name moved in step 2.1 kept its leading underscore (`core.scanner._scan_all_offsets`), which by convention means "private to this module" — wrong for what is now a package API. Kept deliberately so that step 2.1 was a **pure move**, provable by the 549 existing tests passing untouched; renaming touches every call site and deserves its own verifiable step. Also parked here: **`_nearest_idx` was not moved** — four pure lines that belong in `core/`, but untested, and untested code is where a silent break hides. It goes with `views/` (2.4), where its call sites are. **Fix:** rename to public names in one mechanical pass, ideally after 2.4 so the call sites are already sorted. | M2.4 |
...
| DEBT-002 | **P0** | `app.py` is a **3,991**-line procedural script (757 lines CSS) | M2 |
| | | *Re-measured 2026-07-29: 4,230 when the audit counted it, 4,283 before M2 began, **3,991 after step 2.1** extracted `core/` (−292). First reduction since the audit — it had only accreted until now. Four steps of the decomposition remain.* | |
