...
| BUG-020 | ~~**P0**~~ **FIXED 2026-07-30** | **The dashboard froze completely, at 100% CPU, whenever a new snapshot arrived** | Reported repeatedly as "the screen appears frozen" and misdiagnosed twice — first as the work in progress, then as a stale process. Both wrong. **The live-refresh fragment reruns the app when a newer snapshot appears, and `st.rerun()` ABORTS the running script where it stands.** The line recording which snapshot is on screen (`_active_snapshot_id = snapshot_id`) sat ~100 lines further down, so it never ran. The next execution called the poller again at the top, found the same stale value, and rerun again — an infinite loop that never reached the point where anything is drawn. **Every symptom follows:** the page freezes, the charts stop responding to the expiry and strike controls, clicks do nothing, and CPU pins at 100%+ indefinitely. It needs one new snapshot to trigger, which is why the first page load always looked fine and why this read as intermittent. **The code comment three lines above described this exact trap** and guarded only the first-run branch; the newer-snapshot branch had the same hazard and no guard. **Fix:** record the snapshot before rerunning, so the rerun happens once per snapshot. **Why nothing caught it:** `AppTest` does not fire fragment timers at all, so no test could ever have reached this code — `render_check.py` included. Pinned now by `test_the_refresh_poller_adopts_the_snapshot_before_it_reruns`, asserted on the source because there is no seam to call. | — |
...
| DEBT-002 | ~~P0~~ **P2, reduced** | `app.py` is a **2,486**-line script (757 lines CSS) | M2.5 |
| | | *Re-measured 2026-07-30: 4,230 when the audit counted it, 4,283 before M2 began, 3,991 after step 2.1 (`core/`), 3,945 after 2.2 (`dataaccess/`) and 2.3 (`state/`), **2,486 after step 2.4 extracted all six tabs into `views/`** (−1,459 in one step). **Down 42% from the audit's count, and no longer the biggest file in the repo.** One step remains: 2.5 targets under 400 lines. Of what is left, **757 lines are the CSS block** and 115 are `_render_mc_section`, which 2.4 injected rather than moved because relocating it needs a signature change (ADR-037) — so roughly 870 of the remaining 2,486 are already identified and spoken for.* | |
