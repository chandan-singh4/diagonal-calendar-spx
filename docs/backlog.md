...
| DEBT-027 | P1 | **Mission Control functions read module globals instead of the arguments they are given** | Found 2026-07-29 while writing DEBT-026's tests; **two sites**, same defect. (1) `_candidate_signals` calls `db.get_transform_mark_history(config.DB_PATH, ...)`, so no caller can direct it at another database and every test must monkeypatch `config.DB_PATH`. (2) `_build_non_atm_panel` accepts a `dte_by_expiry` **parameter** and passes it nowhere — `_exp_label()` reads a module **global** of the same name, so the parameter is decorative. Neither is a runtime defect (production has one database and one expiry map) but both are hidden dependencies that make the code untestable without patching global state, and both will follow it into `core/` unless changed. **Fix:** take `db_path` and `dte_by_expiry` as real parameters, defaulting to the globals, during the M2 extraction. Cheap then; the tests already written will simplify rather than break. | M2 |
