...
| DEBT-030 | P2 | **The data layer decides what the x-axis looks like** | Opened 2026-07-30 by ADR-033. `dataaccess/queries.py` ends its timestamp handling with `.dt.tz_localize(None)`, producing a naive wall-clock time **because Plotly's rangebreaks require one** — a display decision taken inside the read layer, which is the one thing 2.2 was supposed to separate. Same line appears in `_candidate_signals`. Not a defect today: every consumer is a chart wanting exactly this. **Why it was not fixed in 2.2:** changing it moves every chart's x-axis, and no existing test would catch a one-hour shift — the goldens were captured against the current behaviour. **Fix:** return timezone-aware UTC and localise in `views/`, with a test that pins the displayed hour first. Do it *after* 2.4, when there is one place to localise rather than fifteen. | M2.4 |
| DEBT-027 | P1 → **half closed** | **Mission Control functions read module globals instead of the arguments they are given** | Found 2026-07-29 while writing DEBT-026's tests; **two sites**, same defect. ~~(1) `_candidate_signals` calls `db.get_transform_mark_history(config.DB_PATH, ...)`~~ — **fixed 2026-07-30 in M2 step 2.2 (ADR-033):** it now takes `db_path`, defaulting to the global, and `test_the_database_location_can_be_given_instead_of_patched` proves the argument is honoured by pointing the global at a nonexistent file. All nine `dataaccess/` reads take it as their first argument, enforced by `test_layering.py`. **(2) STILL OPEN: `_build_non_atm_panel`** accepts a `dte_by_expiry` **parameter** and passes it nowhere — `_exp_label()` reads a module **global** of the same name, so the parameter is decorative. Not a runtime defect (production has one expiry map) but a hidden dependency that keeps the pipeline untestable without patching global state. **Fix:** thread `dte_by_expiry` through as a real parameter when the Mission Control pipeline moves — 2.4, since `_exp_label` is a display helper. | M2.4 |
...
| DEBT-011 | P1 | Sidecar JSON (`eligible_history.json` 599 KB, `entry_locks.json`) has no schema, no validation, no atomic write, no backup — **and both paths are relative** | M2.6 |
