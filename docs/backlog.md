...
| ID | P | Item | Milestone |
|---|---|---|---|
| DEBT-026 | **P0** *(part done)* | **The composite Mission Control functions are still unpinned going into M2** | `test_display_golden.py` pins the pure display layer (28 tests, ADR-029). `test_mission_control_golden.py` now pins **`_candidate_signals`** — Duration Active, the ETA, the sparkline and the trend arrow — against a real temporary database (22 tests, 11 injections / 9 caught / 2 proven equivalent, ADR-030). The reusable fixture (`mc_db`, `make_transform_history`) is in `conftest.py` and is the reason the rest is now cheap. **Still unpinned: `_compute_mc_core`, `_build_non_atm_panel`, `_run_mission_control`, and the eleven `_load_*` queries.** `_compute_mc_core` is the highest-value remainder — it holds the rank-*before*-cap decision that determines which opportunities reach the screen at all. **Do it before M2 moves that pipeline, not after.** | M2 (pre-work) |
| DEBT-027 | P1 | **The Mission Control functions take their database path from a module global, not an argument** | Found 2026-07-29 while writing DEBT-026's tests. `_candidate_signals` calls `db.get_transform_mark_history(config.DB_PATH, ...)`, so no caller can direct it at a different database and every test must monkeypatch `config.DB_PATH`. Not a runtime defect — there is only ever one production database — but it is a hidden dependency that makes the function untestable without patching global state, and it will silently follow the code into `core/` unless someone changes it. **Fix:** take `db_path` as a parameter, defaulting to `config.DB_PATH`, during the M2 extraction. Cheap then, and the tests already written will simplify rather than break. | M2 |
