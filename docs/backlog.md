...
| BUG-016 | **P1** | **`get_next_trade_id()` reuses an ID after any deletion, and the reuse raises** | Found 2026-07-26 by the M1.5 tests. `db.py:1043` derives the next ID from `COUNT(*) + 1`, which is not a sequence. Delete any trade that is not the newest and the next generated ID collides with a surviving `trade_id` — the PRIMARY KEY — so `insert_trade()` raises `IntegrityError` and the trade being recorded is lost, showing a raw sqlite error where a saved trade should be. **Reachable today, and scheduled:** `delete_trade()` is wired to a confirmed delete button in `pages/journal.py`, and STATUS.md commits to discarding the 6 practice trades before serious trading resumes — deleting T-002 of 6 and then adding a trade is exactly this sequence. **Fix:** derive from `MAX(trade_id)` or a dedicated counter. Pinned by `test_next_trade_id_reuses_an_id_after_a_deletion`. |
| BUG-015 | P2 | `get_spx_intraday_today()` has no upper date bound | Found 2026-07-26 by the M1.5 tests. The name says "today"; the query says `snapshot_timestamp >= ?` with nothing on the other end (`db.py:938-947`), so asking for an older session returns that session **and every session after it**, concatenated into what the caller plots as a single intraday line. `app.py` is safe only by accident — it always passes the date derived from the *latest* snapshot, which makes the open range coincidentally correct. Any future caller asking for a historical session (backtest replay ENH-007, or a per-day chart) gets silently wrong data with no error. **Fix:** `AND snapshot_timestamp < date(?, '+1 day')`. Pinned by `test_spx_intraday_has_no_upper_bound_on_the_session`. |
| BUG-014 | P3 | Truthiness guards on floats treat a legitimate `0.0` as missing — `db.py` this time | Found 2026-07-26 by the M1.5 tests. Same shape as BUG-007, two sites: `get_prior_session_close()` (`db.py:917`) and `get_eod_spx()` (`db.py:1130`) both do `float(row[...]) if row and row["underlying_price"] else None`, so a stored `0.0` is indistinguishable from no data. A third variant sits in `get_ic_marks()` (`db.py:1210-1212`), where `r["mark"] or 0.0` turns a NULL mark into a real-looking `0.0` and understates `cost_to_close` — and unlike every history query, that function does **not** fall back to the bid/ask midpoint (the fifth duplication site under DEBT-012). No impact today: SPX is never 0, and IC legs sit near the money where quotes are dense. **Fix:** compare `is not None`. Pinned by `test_prior_session_close_reports_a_zero_price_as_missing` and `test_ic_marks_treats_a_null_mark_as_zero`. |
...
| ID | P | Item | Milestone |
|---|---|---|---|
| DEBT-001 | **P0** partly cleared | ~~No automated tests anywhere (9,628 lines)~~ — 265 tests as of 2026-07-26: `iv_engine` at 100%, the journal P&L maths, a scanner golden net, token expiry, and **`db.py` at 100% statement coverage / 24-of-26 mutation score** (M1.5). **Still bare:** `collector.py`, `schwab_client.py`, and the rest of `app.py`. | M1 |
