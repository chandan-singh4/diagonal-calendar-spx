...
# ─────────────────────────────────────────────────────────────────────────────
# Success Message Banner (persists across one rerun via session_state)
# ─────────────────────────────────────────────────────────────────────────────
...
if page_mode == "📊 Overview":
    ...
    if not all_trades:
        ...
        act_col1, act_col2, act_col3, act_col4 = st.columns([3, 1, 1, 5])

        action_trade_id = act_col1.selectbox(
            "Select trade",
            options=trade_ids,
            label_visibility="collapsed",
            key="action_trade_select",
        )
        ...
        if st.session_state["confirm_delete_id"]:
            ...
            st.warning(
                f"⚠️ Are you sure you want to delete **{del_id}**? "
                f"This cannot be undone."
            )
            conf_col1, conf_col2, _ = st.columns([1, 1, 6])
            if conf_col1.button("Confirm Delete", type="primary", key="btn_confirm_delete"):
                db.delete_trade(config.DB_PATH, del_id)
                st.session_state["confirm_delete_id"] = None
                st.session_state["_success_msg"] = f"Trade {del_id} deleted."
                st.rerun()
            if conf_col2.button("Cancel", key="btn_cancel_delete"):
                st.session_state["confirm_delete_id"] = None
                st.rerun()
...
# ─────────────────────────────────────────────────────────────────────────────
# ── MODE: LOG A TRADE  (new entry OR edit existing)
# ─────────────────────────────────────────────────────────────────────────────
    ...
    if is_edit and edit_trade:
        title_col, cancel_col = st.columns([5, 1])
        title_col.subheader(f"Log a Trade — Editing {edit_id}")
        title_col.caption(
            f"Modifying the initial entry for {edit_id}. "
            f"Transformation, IC, and expiration data are unchanged."
        )
    ...
    # ── Parse existing legs for pre-population ────────────────────────────────
    ...
    # ── Form — key changes between new/edit so widgets re-initialise ──────────
    ...
    with st.form(form_key):
        ...
        default_date     = date.fromisoformat(edit_trade["entry_date"]) if is_edit and edit_trade else date.today()
        default_time     = (edit_trade["entry_time"] or "") if is_edit and edit_trade else ""
        default_spx      = float(edit_trade["spx_at_entry"] or 0.0) if is_edit and edit_trade else 0.0
        default_contracts = int(edit_trade["contracts"]) if is_edit and edit_trade else 1
        default_debit    = float(edit_trade["total_debit"]) if is_edit and edit_trade else 0.0
        default_comm     = float(edit_trade["commissions"] or 0.0) if is_edit and edit_trade else 0.0
        default_notes    = (edit_trade["notes"] or "") if is_edit and edit_trade else ""

        entry_date  = c1.date_input("Entry Date", value=default_date)
        entry_time  = c2.text_input("Entry Time (ET)", value=default_time, placeholder="09:34")
        spx_entry   = c3.number_input("SPX at Entry", min_value=0.0, step=0.01, value=default_spx)
        contracts   = c4.number_input("Contracts", min_value=1, step=1, value=default_contracts)

        c1, c2 = st.columns(2)
        total_debit  = c1.number_input("Total Debit / share ($)", min_value=0.0, step=0.01, value=default_debit)
        commissions  = c2.number_input("Commissions / fees ($, optional)", min_value=0.0, step=0.01, value=default_comm)
        ...
        for i in range(4):
            ...
            act_val = leg.get("action", "Sell to Open")
            act_idx = ACTION_OPTS.index(act_val) if act_val in ACTION_OPTS else 0
            action  = c3.selectbox("Action", ACTION_OPTS, index=act_idx, key=f"{form_key}_l{i}_act")
            ...
            leg_data.append({
                "expiry": expiry.isoformat(), "type": ltype,
                "action": action, "strike": strike, "fill": fill,
            })

        st.markdown("**Notes**")
        notes = st.text_area(
            "Trade notes (rationale, market context, observations)",
            value=default_notes,
            height=100,
        )

        submit_label = "💾 Save Changes" if is_edit else "💾 Save Trade"
        if st.form_submit_button(submit_label, use_container_width=True):
            if total_debit <= 0:
                ...
                if is_edit:
                    ...
                    trade_fields["status"]   = "Open"
                    db.insert_trade(config.DB_PATH, trade_fields)
        ...
        with st.form("tf_form"):
            ...
            for i in range(4):
                ...
                action  = c3.selectbox("Action",
                                       ["Sell to Close","Buy to Open","Buy to Close","Sell to Open"],
                                       key=f"tf{i}_act")
