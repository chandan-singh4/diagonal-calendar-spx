...
everything that happens between those numbers and your eyes:
  ...
  * the ORDER opportunity cards appear in        (rank_for_panel)
  * the geometry of the multicolor IV-ratio line (banded_ratio_traces)
  * the text in every cell and glyph on a card   (sparkline, _fmt_*)
  * a card's identity across reruns              (card_key)
...
current behaviour is CORRECT. rank_for_panel's tiering is a judgment call
...
rank_for_panel is exercised against the same real production snapshots the
...
def test_ranking_of_real_snapshot_is_stable(disp, tag):
    ...
    ranked = disp["rank_for_panel"](scanned)
...
def test_ranking_leaves_an_empty_frame_alone(disp):
    ...
    assert disp["rank_for_panel"](empty).empty


def test_ranking_does_not_mutate_its_input(disp):
    """A caller reusing the frame after ranking must see it untouched.

    rank_for_panel copies internally and drops its scratch column. If a future
...
def test_band_crossings_are_interpolated_at_the_exact_threshold(disp):
    ...
    """
    traces = disp["banded_ratio_traces"]([1, 2], [0.60, 1.40])
...
def test_band_crossings_are_ordered_correctly_when_falling(disp):
    ...
    """
    traces = disp["banded_ratio_traces"]([1, 2], [1.40, 0.60])
    xs = traces[0].x
...
def test_only_bands_containing_data_produce_a_trace(disp):
    ...
    traces = disp["banded_ratio_traces"]([1, 2, 3], [0.80, 0.85, 0.90])
...
def test_gaps_are_not_connected_across(disp):
    ...
    """
    traces = disp["banded_ratio_traces"]([1, 2, 3], [0.80, float("nan"), 0.90])
...
def test_flat_segment_produces_no_crossings(disp):
    ...
    traces = disp["banded_ratio_traces"]([1, 2], [1.00, 1.00])
...
def test_value_exactly_on_a_boundary_lands_in_both_bands(disp):
    ...
    """
    traces = disp["banded_ratio_traces"]([1], [1.00])
...
def test_eta_formatting(disp, minutes, expected):
    assert disp["fmt_eta"](minutes) == expected
...
def test_duration_formatting(disp, minutes, expected):
    assert disp["fmt_duration"](pd.Timedelta(minutes=minutes)) == expected


def test_duration_of_nothing_is_a_dash(disp):
    assert disp["fmt_duration"](None) == "—"
    assert disp["fmt_duration"](pd.NaT) == "—"


def test_sparkline_shape(disp):
    """The glyph must track the shape of the series, scaled to its own range."""
    assert disp["sparkline"]([]) == "─"
    assert disp["sparkline"]([1, 2, 3, 4, 5]) == "▁▂▄▆█"
    assert disp["sparkline"]([5, 4, 3, 2, 1]) == "█▆▄▂▁"
    # A flat series has no range to scale against; it must not divide by zero.
    assert disp["sparkline"]([7, 7, 7]) == "▄▄▄"
...
def test_sparkline_downsamples_across_the_whole_series(disp):
    ...
    spark = disp["sparkline"](rise_then_fall, width=10)
...
def test_card_key_is_stable_and_distinguishing(disp):
    ...
    assert disp["card_key"](a) == disp["card_key"](a)
    assert disp["card_key"](a) != disp["card_key"](b)
    # Strikes are normalised to int, so 6000.0 and 6000 are the same card.
    assert disp["card_key"](a) == disp["card_key"]({**a, "put_strike": 6000})


# ─────────────────────────────────────────────────────────────────────────────
# nearest_idx — which strike each dropdown lands on by default
#
# Added with DEBT-028, which moved these four lines out of app.py and noted the
# reason they had been left behind: nothing tested them. They are not decorative.
# This function chooses what the Put and Call selectboxes show on a fresh page
# load, so it decides which position a new session is looking at before anyone
# touches a control. Imported directly — it is pure, and needs no page.
# ─────────────────────────────────────────────────────────────────────────────


# A realistic 50-point SPX ladder. Module level, not a class attribute: ruff
# flags a mutable default there (RUF012), and it is shared and never mutated.
STRIKES = [7300.0, 7350.0, 7400.0, 7450.0, 7500.0]


class TestNearestIdx:

    def test_exact_match_returns_that_strike(self):
        assert nearest_idx(STRIKES, 7400.0) == 2

    def test_picks_the_closer_of_two_neighbours(self):
        assert nearest_idx(STRIKES, 7410.0) == 2
        assert nearest_idx(STRIKES, 7440.0) == 3

    def test_a_target_exactly_between_two_strikes_takes_the_LOWER(self):
        """Not arbitrary trivia — SPX sits exactly between listed strikes often
        enough to matter, and the answer falls out of `min` returning the first
        minimum rather than from a decision. Pinned so a later rewrite using
        `sorted` or `argmin` cannot flip which position the page opens on."""
        assert nearest_idx(STRIKES, 7425.0) == 2

    def test_targets_beyond_either_end_clamp_to_the_ends(self):
        assert nearest_idx(STRIKES, 1.0) == 0
        assert nearest_idx(STRIKES, 99_999.0) == len(STRIKES) - 1

    def test_no_strikes_returns_zero_rather_than_raising(self):
        """The callers feed this straight to a Streamlit `index=`. None would
        raise there; 0 is harmless because an empty list renders no widget."""
        assert nearest_idx([], 7400.0) == 0

    def test_the_offset_the_put_selectbox_actually_uses(self):
        """app.py defaults the put strike to `spx_price - 100`, not to spot.
        Exercised with a real SPX level so the arithmetic is checked in the
        combination that ships, not only in isolation.

        Index 0, not 1: 7408.83 - 100 = 7308.83, which is 8.83 from 7300 and
        41.17 from 7350. Written down because the first version of this test
        asserted 1 and failed — the function was right and the assertion was
        wrong, which is the failure worth having."""
        assert nearest_idx(STRIKES, 7408.83 - 100) == 0
